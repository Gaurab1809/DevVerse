...
const ApplyForMentor = () => {
  ...
  useEffect(() => {
    if (useremail) {
      setMentorInfo((prev) => ({ ...prev, useremail }));
    }
  }, [useremail]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!mentorInfo.linkedin)
      validationErrors.linkedin = "Please provide a LinkedIn profile link.";
    if (!mentorInfo.github)
      validationErrors.github = "Please provide a GitHub profile link.";
    ...
    try {
      ...
      if (response.data.insertedId) {
        toast.success(
          "Successfully applied for mentorship! Await admin approval."
        );
      } else {
  ...
  if (isApplied) {
    return (
      <div className="flex flex-col mx-auto space-y-2  justify-center items-center mt-10">
        <Helmet>
          {" "}
          <title>DevVerse | Apply for Mentor</title>{" "}
        </Helmet>
        <h1 className="text-2xl font-semibold">You have already a Applied!</h1>
        <p>Your application is pending . Please wait for admin approvel.</p>
  ...
  if (userRole === "mentor") {
    return (
      <div className="flex flex-col mx-auto space-y-2  justify-center items-center mt-10">
        ...
        <p>
          Thanks for your hard work and contribution to the DevVerse mentor
          panel.
        </p>
        <p>Best wishes for you!</p>
      </div>
  ...
  if (userRole === "admin") {
    return (
      <div className="flex flex-col justify-center items-center mt-10 mx-auto space-y-2">
        ...
        <p>Only normal member can apply for Mentor</p>
        <p>
          Thanks for your hard work and contribution to the DevVerse Admin
          panel.
        </p>
        <p>Best wishes for you!</p>
      </div>
  ...
  return (
    <section className="mt-10 mx-auto p-8 max-w-md bg-white dark:bg-gray-900 shadow-lg rounded-lg">
      ...
      <h1 className="text-2xl font-semibold text-center mb-8">
        Apply to Become a DevVerse Mentor
      </h1>
