  ...
  const handleDeleteAllNotifications = async () => {
    try {
      const response = await axiosPublic.delete(
        `/deleteAllNotification/${user.email}`
      );
      console.log(response.data);
      notificationRefetch();
    } catch (error) {
    ...
    <div>
      ...
      >
        ...
        {notifications.length > 0 && (
          <span className="absolute top-0 left-6 w-2 h-2 rounded-full bg-emerald-500 ring-1 ring-white bg-red-400"></span>
        )}
      </button>
              ...
              {notifications.map((notification) => (
                ...
                >
                  <div className=" inline-flex items-center justify-between w-full  dark:bg-themeColor dark:text-white ">
                    <div
                      className="inline-flex items-center cursor-pointer hover:bg-blue-50 dark:hover:bg-black  dark:bg-themeColor dark:text-white"
                      onClick={() => {
                        navigate(
                          `/users/${notification?.relatedUserEmail}/followerProfile`
                        );
                      }}
                    >
                      <img
                  ...
                  <p className="mt-1 text-sm  dark:bg-themeColor dark:text-white">
