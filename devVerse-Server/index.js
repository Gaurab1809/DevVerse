...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.${process.env.DB_URI_SECRET_KEY}.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    const database = client.db("DevVerse");
    ...
    // All Operations By Gaurab

    // Import Route

    const SignModal = require("./Gaurab/SignModal")(usersCollection);
    const LeaderBoard = require("./Gaurab/LeaderBoard")(
      postsCollection,
      commentsCollection
    );
    const ArchiveData = require("./Gaurab/ArchiveData")(archiveDataCollection);
    const ReportData = require("./Gaurab/ReportData")(reportDataCollection);
    const LikedPostByUser = require("./Gaurab/LikedPostByUser")(
      postsCollection,
      usersCollection
    );
    ...
    // End Of All Operations By Gaurab
    ...
    console.log("DevVerse successfully connected to MongoDB!");
  } finally {
...
// mongodb
app.get("/", (req, res) => {
  res.send("DevVerse is  on the way");
});

app.listen(port, () => {
  console.log(`DevVerse is running on:${port}`);
});
